...
require_once("beautify_php.class.inc");
 ...
 if (isset($_POST)) {
    ...
    // In PHP 4.1.0 or later, $_FILES should be used instead of $_FILES.
   if (is_uploaded_file($_FILES['file']['tmp_name'])) 
	{
		...
		$file_contents = $beauty->beautify();
		highlight_string($file_contents);
	
		// write file to server
		// open for RW, create if needed
		if ( $filehandle = fopen($file_name, 'w+') ) 
		{
			fwrite( $filehandle, $file_contents );

			if ( fclose($filehandle) )
			{
				echo "File $file_name written successfully<br />Check for file
				duplication";
			}

			else
			{
				echo "File $file_name not written<br />";
			}
		}
	...
	{
        echo "Error: no filename or file too large. Filename: " . $_FILES['file']['name'];
		enterdata();
		exit();
    }
}
...
echo phpBeautify::getVersion();
    ...
    <tr><td>
    <input type="radio" name="braces" value='<?php echo BEAUT_BRACES_PEAR;?>'>Braces <a href="http://pear.php.net/manual/en/standards.php" target="_blank">PEAR</a>-style
    </td><td><input type="radio" checked='checked' name="braces" value='<?php echo BEAUT_BRACES_C;?>'>Braces C-style
    </td></tr>
    ...
    <tr><td>
    <input type="radio" name="indent_mode" value="s">&nbsp;Use Spaces&nbsp;<input type="text" name="indent_width" value=3 size=3 maxlength=2>
	</td></tr>
    <tr><td>
	<input type="radio" name="indent_mode" checked='checked' value="t">&nbsp;Use tabs
    </td></tr>
